<div class="heart-1 position-relative">
    ...
    <div class="box-1 box-private">
        @foreach($courses as $item)
            <img width="50" height="50" src="{{ asset('requirement/uploads/course_photo/' . $item->course_photo) }}" alt=""
                 class="img-fluid img-3 ">
            <div class="content-1">
                <p class="pp" lng-tag="Motion">{{$item->course_detail}}</p>
                <p class="pp-1" lng-tag="Dorothy">{{$item->instructor->name}}</p>
                <p class="pp-2" lng-tag="$19.99">${{$item->price->price}}</p>
            </div>
        @endforeach


        <a href="{{route('likecourseShow')}}"><button type="button" class="btn btn-danger button-1"
